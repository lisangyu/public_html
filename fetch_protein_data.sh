...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <protein_family> <taxonomy> <search_id>" >&2
    exit 1
fi
...
if [ -z "$protein_accessions" ]; then
    echo "No proteins found for family: $protein_family in taxonomy: $taxonomy." >&2
    exit 1
fi
...
for accession in $protein_accessions; do
    ...
    sequence=$(efetch -db protein -id "$accession" -format fasta)
...
if [ -s "$output_file" ]; then
    ...
    echo "Failed to fetch protein sequences." >&2
    exit 1
fi
